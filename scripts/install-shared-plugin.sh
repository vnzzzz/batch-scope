...
CLAUDE_MARKETPLACE_NAME="agent-skills"
...
CODEX_MARKETPLACE_NAME="$(node -e 'const data=JSON.parse(process.argv[1]); process.stdout.write(data.marketplaceName)' "$codex_marketplace_json")"
CODEX_PLUGIN_ID="${PLUGIN_NAME}@${CODEX_MARKETPLACE_NAME}"
CLAUDE_PLUGIN_ID="${PLUGIN_NAME}@${CLAUDE_MARKETPLACE_NAME}"

codex plugin marketplace upgrade "$CODEX_MARKETPLACE_NAME" --json
codex plugin add "$CODEX_PLUGIN_ID" --json

if ! claude plugin marketplace update "$CLAUDE_MARKETPLACE_NAME" >/dev/null 2>&1; then
  claude plugin marketplace add "$claude_marketplace_source" --scope user
fi
if ! claude plugin update "$CLAUDE_PLUGIN_ID" --scope user >/dev/null 2>&1; then
