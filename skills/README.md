...
複数repositoryで再利用する汎用Skillは`vnzzzz/agent-skills`をCodex / Claude Code Pluginとして利用します。
BatchScopeのDev Containerでは`vnzzzz/agentic-development-toolkit`の`agent-dev` FeatureがPlugin bootstrapを担当します。
BatchScope側ではPlugin installer、個別Skill名、Skill一覧、provider repository内部pathを管理しません。
