...
リミットの順位付け（#11）では、全体の最短距離だけに依存せず、確定経路の有無と距離も判断に利用します。
...
| `job_network` | 指定したジョブネットを論理ルートとし、配下集合内の別ノードから依存関係の`to`側として参照されない`job`または`job_network`を配下の入口とする | 配下ジョブから求めた代表リミット |
...
|---|---|---|---|
| `finish_by` | タイムゾーンごと | `businessDayOffset`、`localTime`、`dependencyDistance`、ノードID、リミットID | `ranked_finish_by` |
| `max_elapsed` | 一つのグループ | 設定時間、`dependencyDistance`、ノードID、リミットID | `ranked_max_elapsed` |
| `raw` | 一つのグループ | `dependencyDistance`、ノードID、リミットID | `ranked_raw_limit` |

並び順は、表の左から順に比較します。
`dependencyDistance`は小さい順に比較します。
