...
| `job_network` | 配下で、同じジョブネット内の別ノードから先行関係を受けないノード | 配下ジョブから求めた代表リミット |

ジョブネットを指定した場合は、開始点からジョブネット内をたどり、配下のノードから外部へ出る依存関係を後続検索へつなぎます。
配下のジョブは外部の後続として返しません。

開始点の選び方は、実データで循環や複雑な入れ子を確認しながら実装時に確定します。
開始点を選べない場合でも検索できるよう、配下ジョブを開始点として扱う代替処理を用意します。
...
同じ循環を開始位置だけ変えて複数回返さないよう、循環を表すノード列は一定の規則で並べ直します。
