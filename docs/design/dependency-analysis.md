...
次に、通過するジョブまたはジョブネットが少ない経路、relationを持つ接続が少ない経路を優先し、最後はノードIDとrelation IDで同じ入力に同じ代表経路を選びます。
同じ二ノード間の複数relationは一つの接続へまとめるため、relationレコードの件数自体はこの比較に使いません。
...
scope遷移は循環の検出と経路表示に使いますが、依存relationではないため、経路上では`viaScope`としてrelationと区別します。
