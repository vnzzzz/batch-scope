...
キャンセル、タイムアウト、データベースエラー、内部エラーが発生した場合は、蓄積済みの内容を部分結果として成功扱いせず、検索全体をエラーにします。
...
`ConfirmedDependencyDistance`は、確定経路のうち`dependencyDistance`が最小の経路の距離です。
同じ距離の確定経路が複数ある場合も値は同じであり、どの経路を表示するかは[経路ツリー](#経路ツリー)の規則で選びます。
...
同じ地点まで複数の経路がある場合は、次の順に比較し、最初の経路を代表経路として選びます。

1. `declared`または`confirmed`のrelationとscope遷移だけで到達できる経路を優先する。
2. `dependencyDistance`が小さい経路を優先する。
3. `graphDepth`が小さい経路を優先する。
4. 経路に含まれるノードID列、relation ID列の順に比較し、辞書順で最初の経路を選ぶ。

同じ二ノード間に確定relationとそれ以外のrelationがある場合は、確定relationを使って到達できる接続として扱い、表示時は同じ接続の全relationを`viaRelations`へ含めます。
scope遷移は確定経路に含めますが、`dependencyDistance`と`graphDepth`を増やさず、relation ID列にも加えません。
...
自己ループとして扱うのは依存relationだけです。親子関係の循環は取込時に拒否するため、scope遷移は自己ループになりません。

図の例では、`JOB-A`、`JOB-B`、トリガファイル、`JOB-C`が一つの強連結成分です。
`循環外の後続`は強連結成分に含まれず、探索対象に残ります。

SCC内の表示用ノードID列は辞書順に並べ、SCC同士も先頭のノードIDの辞書順に並べます。
一周分の表示経路は、最小のノードIDから開始し、未表示のノードIDが小さい地点へ順に進んだ後、開始点へ戻ります。
二地点間に複数の進み方がある場合は、辺数、ノードID列、relation ID列の順に比較します。
...
| `cycles` | 到達可能な探索グラフで検出した循環の強連結成分 |
