...
| リミットの保存 | リミットはジョブにだけ保存し、ジョブネットの代表値は配下ジョブから検索時に求める |
| リミットの選定 | BatchScopeが`limit-resolution-v1`に従って行う |
| 循環表示 | 一周分を返し、再登場したノードから先をたどらない |
| 省略区間のID | 最大1,000件を返す |
...
| 循環検出 | 見えない依存関係による無限探索を防ぐため | 一周分を記録し、同じ経路上の再訪を打ち切る |
| 処理上限 | 大規模または異常なグラフからサービスを守るため | 深さ、訪問ノード数、返却件数をコード内の定数で制限する |
...
flowchart LR
    ...
    Q[検索対象] --> D[後続との距離と到達可否を計算]
    F --> R[種類ごとの規則で順位付け]
    D --> R
    R --> O[理由付きで返却]
```

スナップショット作成時には検索の起点が分からないため、Normalizerはリミットを選びません。
BatchScopeが検索時に後続をたどり、公開済みの規則で並べます。
...
| 公開APIの探索件数パラメーター | 利用者が適切な内部上限を判断しにくいため |
...
| ジョブネットの開始点 | ジョブネット指定時の後続探索 |
| 対象検索の最大1,000件 | ページングの必要性 |
| 検索とレスポンスの内部上限 | コード内定数 |
