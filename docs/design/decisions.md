...
単一検索の中央値と並行度4の内部処理のp95は異なる測定であり、HTTP層のDTO組立てとJSON化を含む最終p95はIssue #13で確認します。
...
SCCサイズは、内部処理の中央値1秒以下かつメモリ増分512 MiB以下を基準とし、測定点のうち両方を満たす最大の3,000ノードを上限とします。
...
| 対応規模と検索方式 | Issue #14の測定から、[初期対応規模](../operations.md#初期対応規模)、索引付きSQLiteの継続、検索用の最大4接続を確定し、公開HTTPの最終p95はIssue #10とIssue #13で確認する |
