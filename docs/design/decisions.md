...
初期対応規模は、10,000ノード、25,000 relation、5,000リミット設定、ジョブネット階層深さ64とし、想定同時検索数を4にします。
...
複数読み取り接続へ変えるだけで、並行度2、4、8のレイテンシ中央値は19.1%、23.0%、32.8%短縮し、スループット中央値は23.5%、29.0%、46.4%増えました。
...
複数読み取り接続では並行度2、4、8で接続待ちが発生せず、並行度4のp95は839.499 msで1秒の目標内でした。
並行度8のp95は1,692.664 msで目標を超えたため、測定で目標を満たした4接続を上限とします。

10,000ノード、25,000 relationの`Traverse`、`Scan`、`Build`の中央値は396.929 msでした。
20,000ノード、50,000 relationでは1.125秒となり、HTTP層のDTO組立てとJSON化の前に1秒の目標を使い切ります。
このため、前者をノード数とrelation数の初期上限に採用します。
