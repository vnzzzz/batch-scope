...
初期対応規模は、10,000ノード、25,000 relation、5,000リミット設定、SCCサイズ3,000ノード、ジョブネット階層深さ64とし、想定同時検索数を4にします。
これらは検索時の打切り条件ではなく、全件検索を完了するための取込時の受入条件です。
上限の正本は[運用](../operations.md#初期対応規模)で定めます。
...
検索用SQLiteは複数の読み取り接続で開き、最大接続数を4にします。
単一接続では並行度2から接続待ちが発生し、並行度4から8ではスループットが約3.8 search/sで頭打ちになりました。
複数読み取り接続では並行度2、4、8で接続待ちが発生せず、並行度4における内部処理のp95は1秒の目標内でした。
並行度8における内部処理のp95は目標を超えたため、測定で目標を満たした4接続を上限とします。

単一検索の内部処理（`Traverse`、`Scan`、`Build`）の中央値はSmallで1秒の目標内でしたが、倍の規模ではHTTP層の処理前に目標を超えました。
この単一検索の中央値に基づき、Smallをノード数とrelation数の初期上限に採用します。
単一検索の中央値と並行度4の内部処理のp95は異なる測定です。
HTTP層のDTO組立てとJSON化を含む最終p95は[後続リミット取得のHTTP性能測定結果](../development/limit-analysis-performance.md)で確認済みです。

リミット設定数は、ノード数とrelation数を受入上限に固定した境界測定で、上限の5,000件まで全件を返し、リミット数の増加が内部処理時間の支配要因ではないことを確認しました。
このため、`MaxSnapshotLimits`は5,000を維持します。

SCCサイズは、内部処理の中央値1秒以下を基準とし、測定点のうちこの基準を満たす最大の3,000ノードを上限とします。
`Build`はSCCサイズに対して超線形に増え、次の測定点では時間基準を超えたため、探索グラフの最大SCCサイズを取込時に検査します。
検索時にはSCCサイズによる打切りを行いません。
測定条件と結果は[Issue #32の対応規模境界](../development/testing.md#issue-32の対応規模境界)を参照してください。
...
| SQLiteの切替 | 取込失敗時も現在の検索を続けるため | `importing.db`を作り、検査後に世代別ファイルへ変更して検索先を切り替え、旧世代は参照が0になってから削除する |
...
| 対応規模と検索方式 | Issue #14の測定から、[初期対応規模](../operations.md#初期対応規模)、索引付きSQLiteの継続、検索用の最大4接続を確定し、公開HTTPの最終p95はIssue #10とIssue #13で確認済み |
