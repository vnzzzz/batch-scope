...
単一検索、同時検索、リミット設定数、SCCサイズを別の条件で測定し、それぞれが全件解析と性能目標を満たす範囲を採用しました。
SCCサイズでは経路ツリーの生成時間が超線形に増えたため、検索時に途中で打ち切るのではなく取込時に拒否します。
測定条件と結果は[性能測定結果](../development/performance-measurement.md)を参照してください。
