...
取込ハンドラーは、処理開始からリクエストボディの受信完了までに10分のdeadlineを設定します。
この値は、上限の500 MiBを約0.85 MiB/秒で送信できる余裕を確保しながら、一件だけの取込枠を遅い送信元が保持し続けることを防ぎます。
deadlineを超えた場合はsocketの読み取りを中断し、一時ファイルと取込予約を破棄して`invalid-request`の`400 Bad Request`を返します。
...
取込の受信deadlineは取込枠の占有時間を制限する値であり、検索SLOや取込性能の目標値ではありません。
