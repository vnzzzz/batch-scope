...
- リミット設定の総数またはジョブネット階層深さが初期対応規模を超えた場合に、検査工程で拒否すること
...
Issue #14の測定では、初期対応規模と同じ10,000ノード、25,000 relationの解析全体の中央値が396.929 msでした。
20,000ノード、50,000 relationでは1.125秒となったため、HTTP処理を加える前に後続リミット取得の1秒目標を使い切ります。
この結果に基づき、Smallをノード数とrelation数の受入上限に採用します。

複数読み取り接続では、並行度4の内部解析のp95が839.499 msであり、並行度8のp95は1,692.664 msでした。
この結果に基づき、検索用SQLiteの最大接続数と想定同時検索数を4にします。
リミット設定の総数5,000件とジョブネット階層深さ64階層も取込時に検査し、測定した入力から一つの要素だけが無制限に増える条件を受け入れません。

測定環境、反復数、メモリ、ディスク、接続方式ごとの結果は[性能測定結果](performance-measurement.md)を参照してください。
